    ...
    <div className="glass-card glass-card-hover rounded-2xl p-6 shadow-xl flex flex-col justify-between relative overflow-hidden group">
      <div>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-base font-bold text-on-surface">Recent Analyses</h3>
          <Link
            href="/resumes"
        ...
        {recent.length === 0 ? (
          <div className="py-8 text-center text-on-surface-variant text-xs bg-surface-variant/30 border border-dashed border-outline-variant/30 rounded-xl">
          ...
          <div className="space-y-2.5 my-2">
            {recent.map((r, idx) => (
              <div
                key={r.id}
                className="bg-surface-variant/40 hover:bg-surface-variant/70 border border-outline-variant/30 p-3.5 rounded-xl flex items-center justify-between transition-all duration-200 group"
              >
                <div className="flex items-center gap-3 min-w-0 pr-2">
                  <div className="w-9 h-9 rounded-xl bg-on-surface/10 border border-on-surface/20 flex items-center justify-center text-on-surface shrink-0">
                    <FileText size={18} />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {idx === 0 && (
                        <span className="bg-on-surface/15 text-on-surface border border-on-surface/30 text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0">
                          Latest
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {r.overall_score !== null && (
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[9px] text-on-surface-variant font-mono mt-0.5 uppercase">ATS Score</span>
                    </div>
                  )}
