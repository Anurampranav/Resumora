...
export default function SkillGapAnalysis({ skillGap }: SkillGapAnalysisProps) {
  ...
  return (
    <div className="bg-[#121324]/80 backdrop-blur-xl border border-violet-500/20 rounded-2xl p-6 shadow-xl flex flex-col justify-between hover:border-violet-500/40 transition-all duration-300">
      ...
      {/* Coverage & Headline */}
      <div className="flex items-center gap-5 bg-white/[0.02] border border-white/5 rounded-xl p-3.5 mb-4">
        <div className="relative w-16 h-16 flex items-center justify-center shrink-0">
          ...
          <span className="absolute text-sm font-extrabold text-white">{coverage}%</span>
        </div>

        <div>
          <h4 className="text-xs font-bold text-white mb-0.5">Skill Coverage</h4>
      ...
      <div className="mb-4">
        <h4 className="text-[11px] font-bold text-gray-300 mb-2 flex items-center gap-1.5 uppercase tracking-wider">
          <span className="text-emerald-400">Strong Skills</span> ({strongSkills.length})
        </h4>
        <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pr-1">
          {strongSkills.map((skill) => (
            <span
              key={skill}
              className="bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 text-[11px] font-medium px-2.5 py-0.5 rounded-md hover:bg-emerald-500/20 transition-colors"
            >
              {skill}
            </span>
          ))}
      ...
      <div>
        <h4 className="text-[11px] font-bold text-gray-300 mb-2 flex items-center gap-1.5 uppercase tracking-wider">
          <span className="text-amber-400">Missing Skills</span> ({missingSkills.length})
        </h4>
          ...
          {missingSkills.map((item) => {
            ...
            return (
              <span
                ...
                className={
                  isCritical
                    ? "bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[11px] font-semibold px-2.5 py-0.5 rounded-md"
                    : "bg-orange-500/10 border border-orange-500/20 text-orange-300 text-[11px] font-medium px-2.5 py-0.5 rounded-md"
                }
              >
                {name}
              </span>
